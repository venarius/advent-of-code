...
const data = fs.readFileSync('./testInput.txt', 'utf-8').split('\n')
...
const boards = data.slice(0, data.length - 1).map((line, index) => {
  if (line === '') {
    ...
    return { lines: horizontalBoardLines.concat(verticalBoardLines), hasWon: false }
  }
}).filter(b => !!b)
...
for (const move of moves) {
  for (const board of boards) {
    ...
    if (!!hasWon && !board.hasWon) {
      const sumOther = board.lines.reduce((a, b) => a + b.filter(l => l !== 'x').reduce((c, d) => c + parseInt(d), 0), 0)
      wins.push((sumOther / 2) * parseInt(move))
      board.hasWon = true
    }
  }
}

console.log(wins[wins.length - 1])
